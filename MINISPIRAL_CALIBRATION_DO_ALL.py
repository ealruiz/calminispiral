...
MSNAME = 'E18C21_SGRA.ms'
IMSIZE = 512
Ns = 11 # Nyquist sampling
deg_to_arcsec= 3600
Bmax = 319200 # greatest projected baseline lenght -> highest resolution (plotms UVwave vs amp)
...
# "Corrected data" column: write Residuals (i.e., after subtracting the compact + extended model)
SEARCH_TRANSIENTS = False # If we want to search transients: set TRUE

# **IF REPEATING** this step, AFTER STEP 5, you should use the
# self-calibrated data, so this should be True. Otherwise,
# set it to False:
USE_SELFCAL_DATA = False ### False if NOT repeating steps
##############
...
# to flag bad integrations:
SGRA_MIN = 3.0 ; SGRA_MAX = 5.0

# Export calibrated minispiral visibilities??
# This is NEEDED **IF STEP 5** is going to be run!
EXPORT_MINISPIRAL = True ### False if not step 5
##############

##############
## After running STEP 5,
#    set USE_SELFCAL_DATA = True (STEP 3 config.) and
#    repeat steps [3,4]
##############
...
if 0 in mysteps:
	...
	os.system('rm -rf %s'%MSNAME)
	split(vis='/home/qhr/Documentos/SGRA_MiniSpiral/track-e18c21.polarization-calibrated.APP.ms',
			outputvis=MSNAME, datacolumn='data',
			field='SgrA_star')
...
if 2 in mysteps:
	...
	for i in range(4):
		...
		Sx = beam[0]/2.35482 ### This division depends on the casa version (e.g. correct for casa5, but must be removed for casa4)
		Sy = beam[1]/2.35482
...
if 3 in mysteps:
	...
	for j,ti in enumerate(UT):
		...
		# Loop over spectral windows (i.e. we fit each integratin time and spw independently):
		for k in range(4):
			...
			# Only proceed if there are unflagged visibilities in XX and YY (we have plenty antennas and integration times):
			TWGT = WGT[0,mask2]
			TWGT2 = WGT[3,mask2]
			...
			if Hessian[0,0] != 0.0 and Hessian[1,1] != 0.0:
				...
				#    and impose that the extended component has a constant flux (dividing by the extended flux)
				COR[0,:,mask2] = (DAT[0,:,mask2] - CMPFLX)/EXTFLX
				COR[3,:,mask2] = (DAT[3,:,mask2] - CMPFLX)/EXTFLX
				if SEARCH_TRANSIENTS: # Residuals: we subtract the fitted model visibilities
					COR[0,0,mask2] = DAT[0,0,mask2] - EXTFLX*MOD[0,0,mask2] - CMPFLX
					COR[3,0,mask2] = DAT[3,0,mask2] - EXTFLX*MOD[3,0,mask2] - CMPFLX
	...
	FIT_ARR = [np.array(np.zeros(len(I_FIT[k])), dtype=np.dtype([('JDTime', np.float), ('I Extended', np.float), ('I Compact',np.float),('I LongBas', np.float),('Q', np.float), ('U',np.float),('V',np.float), ('Error Ext.',np.float), ('Error Comp.',np.float), ('Covariance', np.float), ('Error Q',np.float),('Error U',np.float),('Error V',np.float)])) for k in range(4)]
...
if 4 in mysteps:
	...
	for k in range(4):
		fig = pl.figure()
		Tplot = LCData[0][i]['JDTime']/86400.
		T00 = np.floor(np.min(Tplot))
		Tplot -= T00
		Tplot *= 24.
		pl.plot(Tplot,LCData[0][i]['I Extended'],'ob',label='Extended')
		pl.plot(Tplot,LCData[0][i]['I Compact'],'or',label='IF selfcal')
		pl.plot(Tplot,LCData[0][i]['I LongBas'],'og',label='Long Bas.')
		pl.legend(numpoints=1)
		pl.xlabel('UT')
		pl.ylabel('SgrA* Flux Density (Jy)')
		pl.suptitle('REAL DATA',fontsize=25)
		pl.savefig('step3_LCURVES_spw%i.png'%k)
	...
	if EXPORT_MINISPIRAL:
		...
		# Scale the minispiral data to the flux-density average:
		print('Scaling minispiral data')
		tb.open('%s_ExtendedData'%MSNAME,nomodify=False)
		SPI = tb.getcol('DATA_DESC_ID')
		DAT = tb.getcol('DATA')
...
if 5 in mysteps:
	...
	# Self-calibrate:
	gaincal(vis='%s_ExtendedData'%MSNAME,calmode='ap',solint='int',caltable='%s_SELFCAL.tab'%MSNAME[:-3])
	applycal(vis='%s_ExtendedData'%MSNAME, gaintable='%s_SELFCAL.tab'%MSNAME[:-3])
	...
	applycal(vis=MSNAME, gaintable='%s_SELFCAL.tab'%MSNAME[:-3])
